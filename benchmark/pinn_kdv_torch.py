"""
PyTorch PINN solver for KdV equation: u_t + 6*u*u_x + u_xxx = 0
...
def main():
    print("=== PyTorch KdV Solver (Autograd) ===")

    torch.manual_seed(42)
    device = torch.device('cpu')

    # Network: same architecture as C++ version
    layers = [2, 50, 50, 50, 1]
    ...
    for it in range(iterations):
        ...
        # Sample random points in [0,1]x[0,1]
        ...
        # Third-order: u_xxx
        u_xxx_grads = torch.autograd.grad(u_xx, x_t, grad_outputs=torch.ones_like(u_xx),
                                           create_graph=True)[0]
        u_xxx = u_xxx_grads[:, 0:1]

        # PDE residual: u_t + 6*u*u_x + u_xxx = 0
